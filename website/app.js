...
function next(succ = false) {
  ...
  if (succ) {
    ...
    question.innerHTML = "정답: " + answer + "<p>ㅤ</p><p>Correct!</p>";
  } else {
    ctx.fillStyle = "red";
    ctx.fillRect(0, 0, Math.round((canvas.width / 700) * timer), 20);
    question.style.color = "red";
    if (timer >= 700) {
      question.innerHTML = "정답: " + answer + "<p>ㅤ</p><p>Time Out!</p>";
    } else {
      question.innerHTML =
        "정답: " +
        answer +
...
function setQuestion() {
  ...
  if (main instanceof King) {
    ...
    answer = main.name;
    question.innerHTML =
      "다음 설명에 해당하는 나라의 이름은?<p id='lh'>ㅤ</p><p id='lh'>" +
      getRandomElement(main.info, 1).join("</p><p id='lh'>") +
      "</p>";
  }
}
